<div class="w-full bg-white px-2 py-8 ">
    ...
    <div class="w-full flex flex-col lg:flex-row items-start justify-center gap-10 my-5">
        ...
        <div id="available-bookings" class="w-full lg:w-[40%] delay-150 ease-in-out p-0 lg:pr-2 hidden">
            ...
            <div id="booking-items"></div> <!-- Dynamic items will be added here -->
        </div>
        {{-- available bookings --}}
        {{-- <div class="w-full lg:w-[40%] delay-150 ease-in-out p-0 lg:pr-2 hidden">
            <h2 class="text-left text-2xl font-bold text-[#07074D]">Available Bookings</h2>
            
            <div
                class="group flex flex-col items-start justify-between border-b border-[#e0e0e0] py-5 cursor-pointer mb-3">
                ...
                <div
                    class="flex flex-wrap justify-center md:justify-between items-center text-center w-full text-lg text-[#07074D] font-semibold bg-green-400 px-4 py-2 rounded-md gap-5 md:gap-0">
                    <div class="p-2">
                        <p class="font-bold">Deck Seats</p>
                        <p class="text-xs font-bold">(40)</p>
                        <p class="flex flex-row justify-center items-center">500<sub>tk</sub><span
                                class="text-xs">(per)</span></p>
                    </div>
            ...
            try {
                ...
                if (res.data.length > 0) {
                    ...
                    bookings.forEach((booking) => {
                        const formattedDate = booking.departure_date.split(' ')[0]; 
                        const seatsHTML = booking.seats.map(seat => `
                    <div class="p-2">
                        <p class="font-bold">${seat.category}</p>
                        <p class="text-xs font-bold">(${booking.couch_no})</p>
                ...
                `).join('<div class="h-20 w-1 border-l border-[#e0e0e0] hidden md:block"></div>');
                        ...
                        const bookingHTML = `
                    <a href="{{url('/booking/${booking.id}/${booking.departure_point_id}')}}" class="group flex flex-col items-start justify-between border-b border-[#e0e0e0] py-5 cursor-pointer mb-3">
                        ...
                        <div class="flex flex-row justify-between items-start w-full mb-2">
                            <p class="text-sm text-[#07074D]"><span class="text-base font-bold">Date:</span> ${formattedDate}</p>
                            <p class="text-sm text-[#07074D]"><span class="text-base font-bold">Departure At:</span> ${booking.departure_time}</p>
                        </div>
