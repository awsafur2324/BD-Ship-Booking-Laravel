                    ...
                    @foreach ($ship['seats'] as $seat)
                        <div id="tab{{ $loop->iteration }}"
                            ...
                            <div class="flex flex-wrap justify-between items-start my-2 w-full">
                                <h1 class="text-lg text-[#272c39] font-semibold">Select {{ $seat['category'] }} Seat
                                </h1>
                                <h1 class="text-right font-serif text-lg text-[#0a161a] font-semibold">
                                    {{ $seat['seat_price'] }}<sup>Per</sup> BDT</h1>
                            </div>
                            ...
                            <div class="flex flex-col justify-center items-center w-full">
                                @for ($i = 1; $i <= $row; $i++)
                                    <div class="flex flex-row gap-2 justify-between items-center my-2 w-full">
                                        @for ($j = 1; $j <= $col; $j++)
                                            @php
                                                $seat_no = ($i - 1) * $col + $j; // Calculate seat number
                                                $seat_id = $seat_tag . '-' . $seat_no; // Generate seat ID (e.g., seat_map-1)
                                            @endphp
                                            <button id="{{ $seat_id }}" value="{{ $seat_id }}"
                                                ...
                                                class="seat w-full mx-1 p-2 bg-gray-200 border border-gray-400 text-center">
                                                {{ $seat_id }}
                                            </button>
                                        @endfor
                                    </div>
        ...
        <div class="part2 w-full md:w-[40%] pl-0 md:pl-6">
            ...
            <div class="p-2 inter">
                ...
                <div class="">
                    ...
                    <label id="cupponTip" class="block text-left text-[#00000097] text-xs">You have no coupon</label>
...
<script>
    ...
    function updateGrandTotal() {
        ...
        const taxRate = 0; // 10% tax rate (for example)
        const grandTotal = totalPrice + (totalPrice * taxRate);
    ...
    async function submitForm() {
        ...
        console.log('Submitting Seat Selection:', selectedSeats);
        // Submit the form
        showLoader();
